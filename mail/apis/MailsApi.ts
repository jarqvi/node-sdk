...
export class MailsApiRequestFactory extends BaseAPIRequestFactory {
     ...
     * @param body 
     */
    public async editMailServer(mailServerID: string, body?: Model8, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'mailServerID' is not null or undefined
        if (mailServerID === null || mailServerID === undefined) {
            throw new RequiredError("MailsApi", "editMailServer", "mailServerID");
        }
...
export class MailsApiResponseProcessor {
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to editMailServer
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async editMailServerWithHttpInfo(response: ResponseContext): Promise<HttpInfo<PostMails201Response >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: PostMails201Response = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
